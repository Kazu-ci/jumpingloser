...
public class PlayerAttackState : IState
{
    ...
    public void OnExit()
    {
        ...
        // OnUpdate 中の先行フェードが未開始なら、残り時間を見て最終フェードをかける
        float remToClipEnd = Mathf.Max(0f, (float)(actionDuration - elapsedTime));
    ...
    public void OnUpdate(float deltaTime)
    {
        ...
        if (!forceReset && queuedNext && HasNextCombo() && elapsedTime >= chainEndTime)
        {
            CrossfadeToNext();
            return;
        }
...
public class PlayerAttackState : IState
{
    ...
    public void OnUpdate(float deltaTime)
    {
        ...
        float endNorm = (currentAction.endNormalizedTime > 0f) ? Mathf.Clamp01(currentAction.endNormalizedTime) : 1f;
        double endTime = endNorm * actionDuration;
        float rem = Mathf.Max(0f, (float)(endTime - elapsedTime));
        float gate = Mathf.Max(0f, currentAction.blendToNext);
